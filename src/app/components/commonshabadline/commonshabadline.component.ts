...
@Component({
  selector: 'app-commonshabadline',
  templateUrl: './commonshabadline.component.html',
  styleUrls: ['./commonshabadline.component.scss']
})
export class CommonshabadlineComponent implements OnInit {
  @Input() page_info: any;
  @Input() footerColor: string;
  @Input() title: string;
  @Input() subtitle: string;
  @Input() selected_line: string;
  ...
  ngOnInit() {
    ...
    if (url.indexOf('/amrit-keertan') > -1) {
      this.pageType = 'ak'
    }
  ...
  print() {
    switch (this.page_info.scripture) {
      case "ak": {
        window.open(location.href + '/print-view', '_blank', 'height=700,width=700');
        break;
      }
      case "dg": {
        window.open(location.href + '/print-view', '_blank', 'height=700,width=700');
  ...
  getShareLink(item) {
    ...
    if (this.pageType === 'bnl') {
      ...
      shabadLineId = item.shabadlineID;
    }
    if (['ggs', 'dg'].indexOf(this.page_info.scripture) > -1) {
      shabadId = item.shabdID;
      shabadLineId = item.shabdlineID;
    }
    if (this.page_info.scripture === 'ak') {
      shabadId = item.shabad_id;
      shabadLineId = item.shabadlineno;
    }
    return 'http://' + location.host + '/' + this.page_info.base_url + '/' + shabadId + '/line/' + shabadLineId
  }
  ...
  getYoutubeUrl(str) {
    return this.santizer.bypassSecurityTrustResourceUrl('https://www.youtube.com/embed/' + str);
    // return this.santizer.bypassSecurityTrustResourceUrl('http://localhost/payu/reactNative_Capture.mp4');
  }
