...
$(document).ready(function () {
        ...
        remote: {
            ...
            prepare: function (q, rq) {
                ...
                rq.data = {
                    ...
                    // source: 'S01',
                    searchtype: $(':visible[name="Searchtype"]').val(),
                    language: $(':visible[name="language"]').val(),
                    author: $('select:visible[name="author"]').val(),
                    raag: $('select:visible[name="raag"]').val(),
                    // page_from: $("#page_from").val(),
                    // page_to: $("#page_to").val(),
                    // scripture: $("#scripture").val(),
                    // individual_search: $("#individual_search").val(),
                    // bnlSelect: $("#bnlSelect").val()
                    page_from: $('input:visible[name="page_from"]').val(),
                    page_to: $('input:visible[name="page_to"]').val(),
                    scripture: $("#scripture").val(),
                    individual_search: $("#individual_search").val(),
                    bnlSelect: $('select:visible[name="bnlSelect"]').val()
                };
                return rq;
            },
    ...
    $(':visible[name="language"]').change(function (e) {
        var $self = $(this);
        $('#outText').val('');
        $('#outText_ascii').val('');
        if($(':visible[name="language"]').val() === 'ROMAN'){
        ...
        else if($(':visible[name="language"]').val() === 'PUNJABI'){
    ...
    $(':visible[name="ResetSearch"]').click(function () {
        $('#outText').val('');
        $('#outText_ascii').val('');
        $(':visible[name="Searchtype"]').val('FL_begin');
        $(':visible[name="language"]').val('ROMAN');
        $('#outText_ascii').css('font-family', '');
        $('#key_pad_punjabi').hide();
        $('#result_div').hide();
        $('#outText_ascii').parent('.twitter-typeahead').show();
    ...
    $(':visible[name="Searchtype"]').change(function (e) {
        var $self = $(this);
    ...
    function getSearchDataCallback() {
            ...
            $(':visible[name="SearchData"]').typeahead(null, {
                ...
                templates: {
                    suggestion: function (data) {
                        ...
                        if($(':visible[name="language"]').val() == 'PUNJABI-ASC') {
        ...
        else if($(':visible[name="language"]').val() === 'PUNJABI-ASC'){
